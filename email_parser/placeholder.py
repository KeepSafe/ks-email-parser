...
def _save_placeholders_file(placeholders, src_dir):
    fs.save_file(json.dumps(placeholders, sort_keys=True, indent=4), src_dir, PLACEHOLDERS_FILENAME)
...
def _parse_email_placeholders(email_path):
    ...
    return list(set(m.group(1) for m in re.finditer(r'\{\{(\w+)\}\}', content)))
...
def _all_placeholders_for_email_name(locale_placeholders):
    return list(set(reduce(lambda x, y: x + y, locale_placeholders.values())))
...
def generate_config(options):
    ...
    if placeholders and _validate_placeholders(placeholders):
        placeholders = _reduce_to_email_placeholders(placeholders)
        _save_placeholders_file(placeholders, options[consts.OPT_SOURCE])
